...
class MyOwnDataset(Dataset):
    ...
    def process(self):
        ...
        for pcd_name in tqdm(dataset, total=dataset.__len__()):
            ...
            node_feats = torch.tensor(pcd_np[:,1:4]).cuda()
            # Get edge features
            edge_feats = None
            # Get adjacency info
...
def test_performance(dataset, net, device):
    prediction_accuracies = []
    for data in dataset:
        data = data.to(device)
        predictions = net(data)
        ...
        gt_labels = gt_labels[:,None]
        predicted_seg_labels = predictions.argmax(dim=-1, keepdim=True)
        if predicted_seg_labels.shape != gt_labels.shape:
            raise ValueError("Expected Shapes to be equivalent")
        torch_pcd = torch.concat([data.pos.T, predicted_seg_labels.T]).T
        pynt_pcd = PyntCloud()#.add_scalar_field("label")
        pynt_pcd.points = PyntCloud(pd.DataFrame(data.pos.detach().cpu().numpy(), columns = ['x','y','z']))
        pynt_pcd.add_scalar_field("label")

        o3d_pcd = o3d.geometry.PointCloud()
        o3d_pcd.points =  o3d.utility.Vector3dVector(data.pos.detach().cpu().numpy())
        
        o3d_pcd.points = o3d.utility.Vector3dVector(torch_pcd.cpu().detach().numpy())
        o3d.io.write_point_cloud("vis_test.pcd", torch_pcd)
        prediction_accuracies.append(accuracy(predictions, gt_labels))
    return sum(prediction_accuracies) / len(prediction_accuracies)
...
def main(rank, world_size):
    ...
    test_data = MyOwnDataset(
        root="gnn_dataset/test_pyg_data/",
        validation=False,
        pre_transform=pre_transform
        )
    
    train_loader = DataLoader(train_data,  shuffle=True, batch_size=1) #, num_workers=1) #, num_workers=1, batch_size=1)
    validation_loader = DataLoader(validation_data, shuffle=False, batch_size=1) #, num_workers=1) #, num_workers=1, batch_size=1)
    ...
    with tqdm(range(num_epochs), unit="Epoch") as tepochs:
        for epoch in tepochs:
            ...
            test_performance(test_loader,multi_net,device)
