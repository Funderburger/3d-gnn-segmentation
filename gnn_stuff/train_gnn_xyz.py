...
class MyOwnDataset(Dataset):
    ...
    def process(self):
        ...
        for pcd_name in tqdm(dataset, total=dataset.__len__()):
            ...
            pcd_np = np.array([pcd_object.points.label, pcd_object.points.x, pcd_object.points.y, pcd_object.points.z]).T
...
def main(rank, world_size):
    ...
    with tqdm(range(num_epochs), unit="Epoch") as tepochs:
        for epoch in tepochs:
            ...
            train_acc, valid_acc = test(multi_net, train_loader, validation_loader, device)
            ...
            sleep(0.1)
            writer.add_scalars("xyz_run_1",{'train_loss': train_loss,
                                            'train_accuracy': train_acc*100,
                                            'valid_accuracy': valid_acc*100},epoch)
            writer.flush()

            if valid_acc > best_test_acc:
                best_test_acc = valid_acc
                torch.save(multi_net.state_dict(), "gnn_results/checkpoints/best_checkpoint_model_xyz")
        writer.close()
