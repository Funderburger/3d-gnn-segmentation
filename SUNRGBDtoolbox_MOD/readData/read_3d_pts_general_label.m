function [rgb,points3d,points3dMatrix,seglabel]=read_3d_pts_general_label(depthInpaint,K,depthInpaintsize,imageName,anno2dSeg,crop)
    ...
    if ~isempty(anno2dSeg)
        seglabel = anno2dSeg;
    else
        seglabel = zeros(depthInpaintsize(1),depthInpaintsize(2));
    end

    seglabel = reshape(seglabel, [], 1);
    rgb = reshape(rgb, [], 3);
    %3D points
